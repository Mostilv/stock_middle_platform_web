...
const StockChart: React.FC<StockChartProps> = ({
  ...
  const processedData = useMemo(() => {
    ...
    return data.map(item => ({
      time: typeof item.time === 'string' ? item.time : new Date(item.time).toLocaleDateString(),
      open: item.open,
      high: item.high,
      low: item.low,
      close: item.close,
  ...
  }, [chartType, processedData, showVolume, chartColors, theme, showGrid, showDataLabel, showTimeSelector]);
  ...
  if (!data || data.length === 0) {
    return (
      <StockChartContainer $theme={theme} className={className} style={{ width, height }}>
        {title && (
  ...
  return (
    <StockChartContainer $theme={theme} className={className} style={{ width, height }}>
      ...
      <ChartContent>
        <EChart
          option={chartOption}
          height={typeof height === 'number' ? height - (title ? 60 : 0) : height}
