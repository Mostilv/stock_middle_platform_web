...
import { SideChartsContainer } from './SideCharts.styles';

interface SideChartsProps {
  type: 'left' | 'right';
}

const SideCharts: React.FC<SideChartsProps> = React.memo(({ type }) => {
  ...
  const chartOptions = useMemo(() => {
    ...
    return type === 'left' ? leftChartOptions : rightChartOptions;
  }, [type]);

  return (
    <SideChartsContainer $type={type}>
      {chartOptions.map((chart, index) => (
        <SmallChartCard key={index} title={chart.title} option={chart.option} />
      ))}
    </SideChartsContainer>
  );
});

SideCharts.displayName = 'SideCharts';
